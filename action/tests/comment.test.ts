...
function makeScanResult(overrides: Partial<ScanResult> = {}): ScanResult {
  return {
    ...
    categories: [
      ...
      { name: 'spacing', violations: 0, score: 20 },
      { name: 'typography', violations: 0, score: 20 },
      { name: 'responsive', violations: 1, score: 18 },
      { name: 'consistency', violations: 1, score: 18 },
    ],
    filesScanned: 10,
  ...
  it('shows celebration message when no violations', () => {
    const comment = formatComment(
      makeScanResult({
        ...
        topViolations: [],
        categories: [
          { name: 'colors', violations: 0, score: 20 },
          { name: 'spacing', violations: 0, score: 20 },
          { name: 'typography', violations: 0, score: 20 },
          { name: 'responsive', violations: 0, score: 20 },
          { name: 'consistency', violations: 0, score: 20 },
        ],
      }),
      0,
    );
