...
Deterministic design-system, accessibility, **and backend-safety** rules; reproducible attestations; and a commit trailer your merge gate re-verifies. Runs inside the agent loop (MCP server for Claude Code, Cursor, Codex, Windsurf) and at the merge gate (GitHub Action). Catches spacing inconsistencies, typography drift, arbitrary colours, broken responsive layouts, WCAG accessibility failures, missing dark-mode coverage, inline style violations — **and now leaked API keys, SQL injection, shell injection, path traversal, SSRF, open redirects, weak crypto, permissive CORS, disabled TLS, insecure cookies, JWTs without expiry, code-eval sinks, Next.js client-side env leaks, and React hydration mismatches** — in code generated by any AI coding tool, frontend or backend.
...
### Framework stability — Next.js / React (2)

Catches the two most common AI-generated breakages of the Next.js client/server boundary.
...
**Auto-fix coverage:** 14 of 51 rules auto-correct in place (the rest report only — adding a responsive breakpoint, naming a design token, moving a secret to a secret manager, or rewriting a SQL query to use placeholders is a human decision).
