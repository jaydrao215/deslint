...
export const alt =
  'Deslint vs. ESLint — built on, not against. ESLint catches JavaScript bugs; Deslint catches design-system drift in JSX, Tailwind, and inline styles. Honest side-by-side comparison.';

// Comparison-page OG. The verdict frame is "BUILT ON, NOT AGAINST" to
// neutralize the "is this just another plugin" objection — deslint
// installs into the same eslint command, with its own rule layer above
// JS correctness. Reuses the gradient, ProductChip pattern, and brand
// stack from the stylelint OG.
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, marginTop: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
            <ProductChip name="ESLint" accent="#4B32C3" sub="JavaScript correctness" />
          ...
          <div style={{ display: 'flex', marginTop: '20px', fontSize: '19px', lineHeight: 1.5, color: 'rgba(250,250,251,0.65)', maxWidth: '760px', textAlign: 'center', justifyContent: 'center' }}>
            ESLint catches JavaScript bugs. Deslint adds 37 deterministic rules for design tokens, Tailwind drift, WCAG 2.2, and frontend safety — all inside the same eslint command.
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '32px' }}>
          <div style={{ display: 'flex', fontSize: '14px', color: 'rgba(250,250,251,0.45)' }}>
            Side-by-side on six real questions — JS bugs, JSX drift, arbitrary values, WCAG, AI agents, CI.
          </div>
