            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  <tr>
                    <td className="px-5 py-3 text-gray-700">Authored <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">.css</code> / <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">.scss</code> (nesting, ordering, syntax)</td>
                    <td className="px-5 py-3"><VerdictBadge v="stylelint" /></td>
                  </tr>
                  <tr>
                    <td className="px-5 py-3 text-gray-700">Tailwind arbitrary values in JSX (<code className="text-xs text-primary bg-primary-50/60 px-1 rounded">bg-[#1a5276]</code>)</td>
                    <td className="px-5 py-3"><VerdictBadge v="deslint" /></td>
                  </tr>
                  <tr>
                    <td className="px-5 py-3 text-gray-700">Raw hex colors in <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">className</code> / <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">style</code> props</td>
                    <td className="px-5 py-3"><VerdictBadge v="deslint" /></td>
                  </tr>
                  <tr>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-gray-200 bg-white p-5">
                ...
                <p className="text-base text-gray-700 leading-relaxed mb-3">
                  A CSS linter. It parses <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">.css</code>, <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">.scss</code>, <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">.less</code>, <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">.vue</code>, and style blocks, then runs rules against the CSS AST. It catches bad selectors, invalid values, disallowed nesting, ordering problems, and vendor prefixes you forgot to strip.
                </p>
              ...
              <div className="rounded-2xl border border-primary/30 bg-primary-50/30 p-5">
                ...
                <p className="text-base text-gray-700 leading-relaxed mb-3">
                  An ESLint plugin for the AI-coding surface — frontend, backend, and the Next.js client/server boundary. It runs 62 deterministic rules against what AI coding agents actually write: <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">className</code> strings, <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">style</code> props, Tailwind utilities, inline hex colors, plus backend-safety rules (SQL injection, hardcoded secrets, weak crypto, open redirects, path traversal, SSRF) and AI-coding antipatterns (<code className="text-xs text-primary bg-primary-50/60 px-1 rounded">useEffect(async)</code>, empty catches, <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">res.send(err.stack)</code>, <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">localhost</code> URLs shipping to prod).
                </p>
                <p className="text-base text-gray-700 leading-relaxed">
                  Its job is to catch the stuff stylelint structurally can&rsquo;t see: <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">bg-[#1a5276]</code> sitting in a React component, a one-off <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">p-[17px]</code>, a <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">text-blue-500</code> that should have been a token.
                </p>
              </div>
            </div>
          </section>
            ...
            <div className="space-y-8">
              <article>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900">Who catches a raw hex in <code className="text-sm text-primary bg-primary-50/60 px-1 rounded">className=&quot;bg-[#1a5276]&quot;</code>?</h3>
                </div>
                <p className="text-base text-gray-700 leading-relaxed mb-3">
                  Stylelint never sees this. It runs on CSS files, not JSX string literals, so a Tailwind arbitrary value inside a React <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">className</code> is invisible to it — even with <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">postcss-lit</code> or template parsers, the string isn&rsquo;t CSS yet at lint time.
                </p>
                <p className="text-base text-gray-700 leading-relaxed">
              ...
              <article>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900">Who enforces ordering inside an authored <code className="text-sm text-primary bg-primary-50/60 px-1 rounded">tokens.css</code>?</h3>
                </div>
                <p className="text-base text-gray-700 leading-relaxed mb-3">
                  Stylelint. This is its home turf. If you author CSS custom properties, keep a design-token file, or maintain a utility stylesheet, stylelint&rsquo;s rule ecosystem — <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">declaration-block-no-redundant-longhand-properties</code>, <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">order/properties-order</code>, <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">selector-max-specificity</code> — is unmatched.
                </p>
                <p className="text-base text-gray-700 leading-relaxed">
              ...
              <article>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900">Who catches a <code className="text-sm text-primary bg-primary-50/60 px-1 rounded">style={'{'}{'{'} color: &apos;#fa0a3e&apos; {'}'}{'}'}</code> inline prop?</h3>
                </div>
                <p className="text-base text-gray-700 leading-relaxed mb-3">
                  Stylelint can, but only if you wire up a template-literal plugin and the value is in a tagged template or CSS-in-JS string. A plain object-literal <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">style</code> prop on a React element is outside its scope.
                </p>
                <p className="text-base text-gray-700 leading-relaxed">
              ...
              <article>
                ...
                <p className="text-base text-gray-700 leading-relaxed mb-3">
                  Neither does it end-to-end, but the gap is narrower for deslint. Deslint&rsquo;s <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">.deslintrc.json</code> accepts a design-system config (palette, spacing scale, type scale) which the rules then enforce across JSX. Tokens Studio / Style Dictionary export to that shape in one step.
                </p>
                <p className="text-base text-gray-700 leading-relaxed">
              ...
              <article>
                ...
                <p className="text-base text-gray-700 leading-relaxed mb-3">
                  Deslint ships an <Link href="/mcp" className="text-primary font-semibold hover:underline">MCP server</Link> that plugs directly into Claude Code, Cursor, Windsurf, and Codex. The agent can run <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">analyze_project</code>, <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">enforce_budget</code>, and <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">compliance_check</code> before it finalizes a generation — the lint happens inside the authoring loop, not after.
                </p>
                <p className="text-base text-gray-700 leading-relaxed">
              ...
              <article>
                ...
                <p className="text-base text-gray-700 leading-relaxed mb-3">
                  Both. Stylelint has first-class editor plugins, a stable CLI, and mature CI patterns. Deslint ships an ESLint plugin, a standalone CLI (<code className="text-xs text-primary bg-primary-50/60 px-1 rounded">deslint scan</code>), a GitHub Action, and SARIF output for GitHub Advanced Security and Azure DevOps.
                </p>
                <p className="text-base text-gray-700 leading-relaxed">
          ...
          <section className="mb-12">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-5 space-y-4">
              ...
              <div>
                ...
                <p className="text-base text-gray-600 leading-relaxed">
                  You&rsquo;re Tailwind-only, your team barely touches CSS files directly, and most of your drift lives in <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">className</code> strings and inline styles written by AI agents.
                </p>
              </div>
              <div>
        ...
        <FadeIn>
          <section className="mb-12">
            ...
            <p className="text-base text-gray-700 leading-relaxed mb-4">
              The cleanest setup we&rsquo;ve seen: stylelint gates the <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">packages/design-system/**/*.css</code> and <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">globals.css</code> paths, deslint gates the <code className="text-xs text-primary bg-primary-50/60 px-1 rounded">apps/**/*.tsx</code> product surface, and the MCP server runs during AI-assisted authoring. Three gates, zero overlap.
            </p>
