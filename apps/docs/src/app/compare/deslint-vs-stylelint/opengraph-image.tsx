...
export const alt =
  'Deslint vs stylelint — different jobs, not rivals. Stylelint lints authored CSS; Deslint lints the JSX / Tailwind / design-token surface that AI coding agents touch. Honest side-by-side comparison.';

// Comparison-page OG. Reads as a verdict card: two product chips with a
// "DIFFERENT JOBS" badge between them, and the single-sentence summary
// below. Designed to win clicks from the comparison-shopper crowd
// without picking a fight (the actual page is honest about which tool
// wins which question).
export default async function CompareOpenGraphImage() {
  ...
  return new ImageResponse(
    (
      ...
      >
        {/* Top — brand mark + comparison badge */}
        ...
        {/* Center — two product chips with "vs" between, then the verdict */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, marginTop: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
            <ProductChip name="Deslint" accent="#534AB7" sub="JSX · Tailwind · tokens" />
            <div style={{ display: 'flex', fontSize: '34px', fontWeight: 800, color: 'rgba(250,250,251,0.55)', letterSpacing: '-0.02em' }}>
              vs
            </div>
            <ProductChip name="stylelint" accent="#475569" sub="Authored CSS / SCSS" />
          </div>
          ...
          <div style={{ display: 'flex', marginTop: '20px', fontSize: '19px', lineHeight: 1.5, color: 'rgba(250,250,251,0.65)', maxWidth: '720px', textAlign: 'center', justifyContent: 'center' }}>
            stylelint lints authored CSS. Deslint lints the JSX / Tailwind / design-token surface that AI coding agents actually touch. Most teams shipping modern frontend should run both.
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '32px' }}>
          <div style={{ display: 'flex', fontSize: '14px', color: 'rgba(250,250,251,0.45)' }}>
            Side-by-side on six real questions — authored CSS, JSX drift, arbitrary values, tokens, CI, AI agents.
          </div>
