...
- **\`verify_shell_exec\`** — **Agent Action Firewall** pre-execution gate. Agent calls before running any shell command; server reads \`.deslint/policy.yml\` and returns \`allow\` | \`warn\` | \`deny\` + reason (\`denylist\`/\`allowlist\`/\`default\`/\`builtin:<id>\`/\`no-policy\`) + matched pattern. Built-in detection for \`rm -rf /\`, \`curl | sh\`, reverse shells, history rewrites, miners, sudo, process substitution. Sub-1 ms warm; identical-command cache hit instant. Same input, same verdict.
