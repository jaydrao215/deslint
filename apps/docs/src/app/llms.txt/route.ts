...
- **\`verify_shell_exec\`** — **Agent Action Firewall** pre-execution gate. Agent calls before running any shell command; server consults \`.deslint/policy.yml\` and returns deterministic \`allow\` | \`warn\` | \`deny\` verdict + reason (\`denylist\`/\`allowlist\`/\`default\`/\`builtin:<id>\`/\`no-policy\`) + matched pattern. Built-in dangerous-pattern detection: \`rm -rf /\`, \`curl | sh\`, reverse shells, history rewrites, miners, sudo, process substitution. Sub-1ms warm; identical-command cache hit instant. The deterministic guardrail AI agents need to be trusted in production.
