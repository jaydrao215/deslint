# Vizlint Sprint Plan v1.3 — KPMG 7-Moat Defensibility Overlay

> **Apply ON TOP of** v1.0 (`vizlint-sprint-plan.txt`) + v1.1 (`vizlint-sprint-plan-v1.1-update.md`) + v1.2 (`vizlint-sprint-plan-v1.2-update.md`).
...
> **Authoritative status tracker:** `VIZLINT-EXECUTION.md` Section 15.
...
| 7 | Platform Embedding (`@vizlint/core`) | Strategic endgame | Embedded in Lovable/Bolt/v0 generation pipelines |
...
| **VIZ-028** | `vizlint trend` command (history, deltas, regression alerts) | 5 | ✅ | `packages/cli/src/trend.ts` |
...
2. Quality Gates — `enforce` defaults to `false`; failures are warn-only until the user explicitly opts in via `.vizlintrc.json → qualityGate.enforce: true`.
3. Trend — read-only new command over existing `.vizlint/history.json`. Zero writes to new files.
...
5. Compliance Report — net-new `vizlint compliance` command. Existing scan/fix/trend output untouched.

**Exit criteria (met):** A team can install Vizlint, set a quality gate, track trends, import W3C tokens, and export a WCAG compliance report — all without breaking existing v0.1.0 users. Full repo test suite green (82 shared + 102 CLI + plugin + action + mcp).
...
| **P2-3** | Embeddable `@vizlint/core` pure-function API | 7 | 3 weeks | Extract analysis logic from CLI into pure functions. No CLI deps, no filesystem deps. |
...
**Exit criteria:** Vizlint can analyze a 2,000-file project, detect cross-file inconsistencies, attribute violations to AI generation, and produce a report that enterprise design teams act on.
...
- **Stage 2 items in `VIZLINT-EXECUTION.md` Section 11** (HTML parser fix, Angular rule parity, MCP live validation, accessibility expansion, CSS scanning, Figma Variables, cross-file engine) remain valid. KPMG Phase 2 absorbs the cross-file engine and Figma import items; the a11y expansion becomes P2-5.
- **Stage 3 items in `docs/vizlint-sprint-plan-v1.2-update.md`** — Design Debt Scoring, Trend API, HTML/PDF compliance report, WCAG conformance mapping — have all been **pulled forward into KPMG Phase 1** and are ✅ shipped.
...
**Authoritative status tracker:** `VIZLINT-EXECUTION.md` Section 15 has the live mapping between moats, phases, story IDs, and shipped-vs-pending status. Whenever this doc and Section 15 disagree, Section 15 wins.
...
1. **`VIZLINT-EXECUTION.md`** (authoritative, trust-first execution plan, KPMG status tracker in Section 15)
2. `SPRINT-LOG.md` (per-story "Did / Will do / Blockers" history)
3. `docs/vizlint-sprint-plan.txt` (v1.0 base plan)
4. `docs/sprint-plan-v1.1-changes.md` (v1.1 delta)
5. `docs/vizlint-sprint-plan-v1.1-update.md` (v1.1 full overlay)
6. `docs/vizlint-sprint-plan-v1.2-update.md` (v1.2 full overlay — L3-L5 vision)
7. **`docs/vizlint-sprint-plan-v1.3-kpmg-moats.md`** (this doc — KPMG defensibility overlay)

When they conflict, higher-numbered layers win. `VIZLINT-EXECUTION.md` overrides all sprint plans for sequencing.
