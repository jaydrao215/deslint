// Duplicated from @deslint/eslint-plugin/src/utils/color-map.ts rather than
// imported — the CLI must generate the report using only its own compiled
// output, so the palette is inlined to keep the dependency graph one-way.
export const TW_COLOR_HEX: Record<string, string> = {
...
export function twClassToHex(cls: string): string | null {
