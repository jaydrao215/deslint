...
describe('calculateDebt', () => {
  ...
  it('estimates effort using per-rule minute calibration', () => {
    const debt = calculateDebt(
      makeLintResult({
        'vizlint/no-arbitrary-spacing': 10, // 10 * 2 = 20m
        'vizlint/responsive-required': 3,   // 3 * 10 = 30m
      }),
    );
  ...
  it('groups debt by score category', () => {
    const debt = calculateDebt(
      makeLintResult({
        'vizlint/no-arbitrary-colors': 5,    // colors: 5*2 = 10
        'vizlint/no-arbitrary-spacing': 10,  // spacing: 10*2 = 20
        'vizlint/no-inline-styles': 4,       // consistency: 4*5 = 20
      }),
    );
  ...
  it('sorts breakdown by total effort descending', () => {
    const debt = calculateDebt(
      makeLintResult({
        'vizlint/no-arbitrary-spacing': 10, // 20m
        'vizlint/max-component-lines': 2,   // 60m
        'vizlint/no-arbitrary-colors': 5,   // 10m
      }),
    );
    expect(debt.breakdown[0].ruleId).toBe('vizlint/max-component-lines');
    expect(debt.breakdown[0].totalMinutes).toBe(60);
    expect(debt.breakdown[1].ruleId).toBe('vizlint/no-arbitrary-spacing');
    expect(debt.breakdown[2].ruleId).toBe('vizlint/no-arbitrary-colors');
  });

  it('skips rules with zero violations', () => {
    const debt = calculateDebt(
      makeLintResult({
        'vizlint/no-arbitrary-spacing': 5,
        'vizlint/no-arbitrary-colors': 0,
      }),
    );
    expect(debt.breakdown).toHaveLength(1);
    expect(debt.breakdown[0].ruleId).toBe('vizlint/no-arbitrary-spacing');
  });

  it('uses default effort for unknown rule ids', () => {
    const debt = calculateDebt(makeLintResult({ 'vizlint/custom-rule': 4 }));
  ...
  it('rounds totalHours to one decimal', () => {
    const debt = calculateDebt(
      makeLintResult({ 'vizlint/no-arbitrary-spacing': 31 }), // 62m = 1.0333h
    );
    expect(debt.totalHours).toBe(1);
  });

  it('exposes calibration table for inspection', () => {
    expect(RULE_EFFORT_MINUTES['vizlint/no-arbitrary-spacing']).toBe(2);
    expect(RULE_EFFORT_MINUTES['vizlint/responsive-required']).toBe(10);
    expect(RULE_EFFORT_MINUTES['vizlint/max-component-lines']).toBe(30);
  });
});
